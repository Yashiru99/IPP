...
static Poly MakePolyHelper(poly_exp_t dummy, ...) {
    ...
    while (true) {
        va_arg(list, Poly);
        if (va_arg(list, poly_exp_t) < 0) // Mine: Dopóki są jakieś argumenty to je przyjmujemy
            break;
        count++;
    }
    va_start(list, dummy);
...
int main() {
    ...
    assert(SimpleAtTest());
    assert(OverflowTest());*/
    /* Poly example1 = P(P(C(1), 0, C(1), 1), 0, C(1), 1);
    Poly example2 = P(P(C(1), 0, C(-1), 1), 0, C(-1), 1);
    Poly example3 = C(2);
    Poly c = P(C(2), 1);
    PolyPrint(example1);
    putchar('\n');
